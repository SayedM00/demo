	...
	foreach ( $atts['icons'] as $item ) {
		...
		if ( $atts['type'] == 'icons-list' ) {
			...
			if ( empty( $item['href'] ) ) {
				...
				echo '<li class="'. esc_attr($li_class) .'"><a href="'. esc_url( $item['href'] ) .'"><span class="ltx-social-color '. esc_attr( $item['icon_fontawesome'] ) .'"></span><span class="head">'. wp_kses_post( str_replace( array('{{', '}}'), array('<strong>', '</strong>'), $item['header'] ) ) .'</span></a></li>';
			}
		}
			else {
